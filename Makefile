...
major:
	...
	git tag --sign -m "$$NEW major release" $$NEW
minor:
	@set -e;\
	export NEW=$$(echo $(TAG) | semver -release minor| jq -r .canonical | tee /dev/stderr); \
	git tag --sign -m "$$NEW minor release" $$NEW 
patch:
	@set -e;\
	export NEW=$$(echo $(TAG) | semver -release patch| jq -r .canonical | tee /dev/stderr); \
	git tag --sign -m "$$NEW patch release" $$NEW

semver:
	@git tag --sign -f -m '$(TAG)' "$$(echo '$(SEMVERS)' | jq -r .major | tee /dev/stderr)"
	@git tag --sign -f -m '$(TAG)' "$$(echo '$(SEMVERS)' | jq -r .majorminor | tee /dev/stderr )"
